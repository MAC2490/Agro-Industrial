<?php 
    ...
    if (!empty($_POST['dataSearch'])) {
        ...
        WHERE insumos_x_cultivos.id_finca = cultivos_finca.id_finca AND cultivos.nombre LIKE LOWER('%$dataSearch%') OR fincas.nombre LIKE LOWER('%$dataSearch%') OR insumos.name LIKE LOWER('%$dataSearch%')");
        $data = $list->fetchAll();
